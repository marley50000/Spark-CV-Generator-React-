  ...
  return (
    <div className="p-8 text-sm text-gray-800 bg-white flex font-sans">
      ...
      <div className="w-1/3 bg-gray-100 p-6">
        <h1 className="text-3xl font-bold text-indigo-700">{personalDetails.fullName}</h1>
          ...
          {experience.map((exp) => (
            <div key={exp.id} className="mb-4">
              ...
              <div className="flex justify-between items-baseline text-sm">
                <p className="font-semibold text-indigo-600">{exp.company}, {exp.location}</p>
        ...
        <Section title="Education" icon={<GraduationCapIcon />}>
          {education.map((edu) => (
            <div key={edu.id} className="mb-2">
              ...
              <div className="flex justify-between items-baseline text-sm">
                 <p className="font-semibold text-indigo-600">{edu.degree}, {edu.fieldOfStudy}</p>
...
const Section: FC<{ title: string, icon: React.ReactNode, children: React.ReactNode }> = ({ title, icon, children }) => (
  <div className="mb-6">
    <h3 className="text-lg font-bold text-gray-800 border-b-2 border-indigo-200 pb-1 mb-3 flex items-center gap-2">
...
const ContactItem: FC<{ icon: React.ReactNode, text: string, href?: string }> = ({ icon, text, href }) => (
    <div className="flex items-center gap-2 mb-2 text-sm">
        <span className="text-indigo-600 w-4 h-4">{icon}</span>
        {href ? <a href={href} target="_blank" rel="noopener noreferrer" className="text-gray-700 hover:text-indigo-600">{text}</a> : <span className="text-gray-700">{text}</span>}
    </div>
);
